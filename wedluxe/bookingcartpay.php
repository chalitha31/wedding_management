...
<body>
    ...
    if ($Tid == "0") {
        ...
        $Hresult =   Database::search("SELECT * FROM `hotel_booking`  WHERE  `user_email` = '" . $_SESSION["user_email"] . "' AND `pay_id` = '2' ORDER BY `id` DESC LIMIT 1");
        $nrow = $Hresult->num_rows;
        if ($nrow == "1") {
            $data = $Hresult->fetch_assoc();
            $hotelrs = Database::search("SELECT `packages`.`price`,`hotel_img_logo`.`logo` FROM `packages` INNER JOIN `hotel_img_logo` ON `packages`.`hotel_id` = `hotel_img_logo`.`hotel_id`  WHERE `packages`.`id` = '" . $data["packag_id"] . "'");
            $Alldata = $hotelrs->fetch_assoc();
        }
    } elseif ($Tid == "2") {

        $Presult =  Database::search("SELECT * FROM `photography_booking` WHERE  `user_email` = '" . $_SESSION["user_email"] . "' AND `pay_id` = '2' ORDER BY `id` DESC LIMIT 1");
        $nrow = $Presult->num_rows;
        if ($nrow == "1") {
            $data = $Presult->fetch_assoc();
            $photors = Database::search("SELECT `photography_package`.`price`,`photography__img_logo`.`logo` FROM `photography_package` INNER JOIN `photography__img_logo` ON `photography_package`.`photography_id` = `photography__img_logo`.`photography_id`  WHERE `photography_package`.`id` = '" . $data["packag_id"] . "'");
            $Alldata = $photors->fetch_assoc();
        }
    } elseif ($Tid == "1") {

        $Dresult   = Database::search("SELECT * FROM `dj_booking` WHERE  `user_email` = '" . $_SESSION["user_email"] . "' AND `pay_id` = '2' ORDER BY `id` DESC LIMIT 1");
        $nrow = $Dresult->num_rows;
        if ($nrow == "1") {
            $data = $Dresult->fetch_assoc();
            $djrs = Database::search("SELECT `dj_package`.`price`,`dj_img_logo`.`logo` FROM `dj_package` INNER JOIN `dj_img_logo` ON `dj_package`.`dj_id` = `dj_img_logo`.`dj_id`  WHERE `dj_package`.`id` = '" . $data["packag_id"] . "'");
            $Alldata = $djrs->fetch_assoc();
        }
    } elseif ($Tid == "3") {

        $Dresult =  Database::search("SELECT * FROM `vehical_booking` WHERE `user_email` = '" . $_SESSION["user_email"] . "' AND `pay_id` = '2' ORDER BY `id` DESC LIMIT 1");
        $nrow = $Dresult->num_rows;

        if ($nrow == "1") {
            $data = $Dresult->fetch_assoc();
            $djrs = Database::search("SELECT `vehicles_img_logo`.`logo` FROM `vehicles_details` INNER JOIN `vehicles_img_logo` ON `vehicles_details`.`id` = `vehicles_img_logo`.`vehical_details_id`  WHERE `vehicles_details`.`id` = '" . $data["vehical_detils_id"] . "'");
            $Alldata = $djrs->fetch_assoc();
        }
    }
            ...
            <div class="pay-card">
                <div class="pay-con">
                    ...
                    <div class="pay-con-details">
                        ...
                        <div class="card-package-price">Price : <span class="pay-price"><?php echo $pay ?></span></div>
                ...
                if ($Tid == "3") {
                    $tpric = "0";
                ?>
                    <div class="pay-btn" onclick="bookingpay('<?php echo $Tid ?>','<?php echo $data['id'] ?>','<?php echo $pay ?>','<?php echo $tpric ?>')">Pay</div>
                <?php
                } else {
                ?>
                    <div class="pay-btn" onclick="bookingpay('<?php echo $Tid ?>','<?php echo $data['id'] ?>','<?php echo $pay ?>','<?php echo $Alldata['price'] ?>')">Pay</div>
                <?php
                }
                ?>
