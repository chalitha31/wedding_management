...
if (empty($mobile)) {
    ...
    if ($n == 1) {
        ...
        $bookdaters = Database::search("SELECT * FROM `hotel_booking` WHERE `wedding_date` = '" . $weddingDate . "' AND `packag_id` = '" . $pcid . "' AND `order_status` = 'confirmed' ");
        ...
        if ($num > 0) {
            ...
            if ($found) {
                ...
                Database::iud("INSERT INTO `hotel_booking` (`user_email`,`mobile`,`wedding_date`,`time_id`,`packag_id`,`order_id`,`order_date`,`pay_id`) VALUES('" . $email . "','" . $mobile . "','" . $weddingDate . "','" . $condition . "','" . $pcid . "','" . $order_id . "','" . $orderDate . "','2')");
