...
$monthDuplirs = Database::search("SELECT  `booking_date` FROM `dj_booking` WHERE `packag_id` = '" . $pid . "' AND `order_status` = 'confirmed'  GROUP BY `booking_date` HAVING COUNT(booking_date) > 1");
...
$monthrs = Database::search("SELECT DISTINCT  `booking_date` FROM `dj_booking` WHERE `booking_date` > '" . $date . "' AND `packag_id` = '" . $pid . "' AND `order_status` = 'confirmed' ORDER BY `booking_date` ASC");
...
if ($numr > 0) {
    ...
    for ($x = 0; $x < $numr; $x++) {
        ...
        $timers = Database::search("SELECT   `time_id` FROM `dj_booking` WHERE `booking_date` = '" . $bookingData["booking_date"] . "' AND `packag_id` = '" . $pid . "' AND `order_status` = 'confirmed'");
