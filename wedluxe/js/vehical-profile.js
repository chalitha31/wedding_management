...
function bookNow(id, email) {
    ...
    let extraday = document.getElementById("extraday");
    ...
    form.append("extraday", extraday.value);
    form.append("vid", id);
    ...
    r.onreadystatechange = function() {
        ...
        if (r.readyState == 4) {
            ...
            if (t == "success") {
                ...
                window.location = "bookingcartpay.php?pay=" + price + "&Cid=" + '3';
    ...
    r.open("POST", "vehicalBookingProcess.php", true);
    r.send(form);
...
function wedDate(vid) {
    ...
    r.open("GET", "vehicalbookedDaysProcess.php?month_id=" + selectedMonth + "&vid=" + vid, true);
    r.send();
}
...
function setColorToTable() {
    ...
    for (let item of tableDataArray) {
        if (item.textContent == "Booked") item.style.color = "#ff4e4e";
