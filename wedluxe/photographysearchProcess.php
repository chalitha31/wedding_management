...
$query = "SELECT * FROM `photography` INNER JOIN `photography__img_logo` ON `photography`.`id` = `photography__img_logo`.`photography_id`  ";
...
if (!empty($sText)) {
    ...
    $query .= " WHERE `name` LIKE '%" . $sText . "%' AND `photography`.`status` = '0'";
} else {

    $query .= " WHERE  `photography`.`status` = '0'";
}
...
?>
        ...
        <div class="photograpy-item-block" onclick="photographyDetails(<?php echo $ps['id'] ?>)">
            <div class="photograpy-item-image-holder">
                <img class="photograpy-item-image" src="images/photographers/<?php echo $ps["image"] ?>" alt="">
            </div>
            <h3 class="photograpy-name"><?php echo $ps["name"] ?></h3>
            <div class="photograpy-company"><i></i><?php echo $ps["company_name"] ?></div>
        </div>
