...
if (empty($mobile)) {
    ...
    if ($n == 1) {
        ...
        $bookdaters = Database::search("SELECT * FROM `dj_booking` WHERE `booking_date` = '" . $weddingDate . "' AND `packag_id` = '" . $pcid . "' AND `order_status` = 'confirmed'");
        ...
        if ($num > 0) {
            ...
            if ($found) {
                ...
                Database::iud("INSERT INTO `dj_booking` (`user_email`,`mobile`,`booking_date`,`time_id`,`packag_id`,`order_id`,`order_date`,`pay_id`) VALUES('" . $email . "','" . $mobile . "','" . $weddingDate . "','" . $condition . "','" . $pcid . "','" . $order_id . "','" . $orderDate . "','2')");
